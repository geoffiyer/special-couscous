...
//Added by Geoff:
#undef OMP   // I don't care about parallel right now

using namespace std;

Nystrom::Nystrom(){};
Nystrom::~Nystrom(){};
void GloriaWeightMatrix(const double* super_test, const vector<int>& myvector, double* Wxx, int m, double* Wyy, int N, int d, double sigma);

void GeoffWeightMatrix(const double* super_test, const vector<int>& myvector, double* Wxx, int m, double* Wyy, int N, int dim1, int dim2, double sigma);
...
void Nystrom::Nystrom_algorithm(const double * super_test, int m, double sigma,
    int N, int dim1, int dim2, double * V, double * D)
{
    int d = dim1+dim2;
    ...
    GloriaWeightMatrix(super_test, myvector, Wxx,m,Wxy,N,d,sigma);
//    GeoffWeightMatrix(super_test, myvector, Wxx,m,Wxy,N,dim1, dim2,sigma);
    ...
    // Geoff says TODO: Change this? I want to normalize earlier I think.
    //      Honestly not really sure I don't follow the math yet.
    for(int i= 0; i<m; i++){
...
void GloriaWeightMatrix(const double* super_test, const vector<int>& myvector, double* Wxx, int m, double* Wxy, int N, int d, double sigma) {
...
void GeoffWeightMatrix(const double* super_test, const vector<int>& myvector, double* Wxx, int m, double* Wxy, int N, int dim1, int dim2, double sigma) {
    ...
    if(dim2 == 0)
        ...
        int inca = 1, incb = 1;
        double d1_num_1, d1_num_2, d1_num_3, d1_dd;
        double d2_num_1, d2_num_2, d2_num_3, d2_dd;
        double dd;
        for(int i = 0; i<m; i++){
            int temp_1 = myvector[i]*d;
            d1_num_2 = ddot_(&dim1, super_test+temp_1, &inca, super_test+temp_1, & incb);
            d2_num_2 = ddot_(&dim2, super_test+temp_1+dim1, &inca, super_test+temp_1+dim1, & incb);
            for(int j = 0; j<m; j++){
                int temp_2 = myvector[j]*d;
                ...
                d1_dd = 1-d1_num_1/sqrt(d1_num_2*d1_num_3);
                d2_num_1 = ddot_(&dim2, super_test+temp_1+dim1, &inca, super_test+temp_2+dim1, & incb);
                d2_num_3 = ddot_(&dim2, super_test+temp_2+dim1, &inca, super_test+temp_2+dim1, & incb);
                d2_dd = 1-d2_num_1/sqrt(d2_num_2*d2_num_3);
                dd = max(d1_dd, d2_dd);
                Wxx[i*m+j]=exp(-dd/sigma);
            }
        }
