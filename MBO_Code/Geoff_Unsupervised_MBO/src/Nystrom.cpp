    ...
    int N, int dim1, int dim2,double * V, double * D)
{
    ...
    //         dd = max(1-d1_num_1/sqrt(d1_num_2*d1_num_3),
    //                  1-d2_num_1/sqrt(d2_num_2*d2_num_3));
    ...
    for(int i=0; i<m*m; i++)
        Wxx[i] = -1*max(Wxx[i], Wxx2[i]);

    for(int i=0; i<m*(N-m); i++)
        Wxy[i] = -1*max(Wxy[i], Wxy2[i]);
...
#ifdef PRINT_WEIGHT
    cout << "Printing sing values to s1.txt\n";
    wout.close();
    wout.open("/home/gsiyer/Schoolwork/Chanussot/MBO_Code/Output/s1.txt");
    wout << fixed << setprecision(7);
    if(wout.fail()) {
        cout << "But it didn't open!!\n";
    }
    for(int i=0; i < m; ++i) {
        assert(!isnan(s1[i]) && !isinf (s1[i]) && !isnan(s2[i]) && !isinf (s2[i]));
