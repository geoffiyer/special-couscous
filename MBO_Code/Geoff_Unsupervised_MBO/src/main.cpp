...
int main(int argc, const char * argv[]) {
    if (argc<8) {
        ...
        printf("Usage: %s n dt lambda sigma input_image_string dim1 dim2\n", argv[0]);
    } else {
        srand(time(NULL));
        ...
        if (d<4) {
            ...
            // consistent with the hyperspectral image
            sigma = sigma * 100.0;
            lambda = lambda / 100.0;
        ...
        // Geoff's version:
        N_obj.Nystrom_algorithm(feature_vec,m,sigma,N,dim1,dim2,V,D);
        cout << "end Nystrom algorithm"<<endl;

#ifdef PRINT_EIGENFUNCTIONS
