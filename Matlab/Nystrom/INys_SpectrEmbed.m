...
function [V,L,scaling] = INys_SpectrEmbed(data, m, normChoice)
...
if(strcmp(normChoice,'angle'))
    E = angledist(data,center);
elseif(strcmp(normChoice,'knn'))
    
else
    E = sqrt(sqdist(data', center'));
end
scaling = std(E(:));
E = exp( - E / scaling);
W = E(dex(1:m),:);
